namespace MassTransit.DapperIntegration.Saga
{
    ...
    public class DapperDatabaseContext<TSaga> :
        ...
        IDisposable
    {
        readonly HanaConnection _connection;
        readonly SemaphoreSlim _inUse;
        readonly HanaTransaction _transaction;

        public DapperDatabaseContext(HanaConnection connection, HanaTransaction transaction)
        {
            ...
            where T : class, ISaga
        {
            ...
            try
            {
                return await _connection.QuerySingleOrDefaultAsync<T>(
                    $"SELECT * FROM {GetTableName<T>()} WHERE CorrelationId = ?correlationId? FOR UPDATE",
            ...
            where T : class, ISaga
        {
            ...
            try
            {
                return await _connection.QueryAsync<T>($"SELECT * FROM {tableName} {whereStatement} FOR UPDATE" , parameters, _transaction)
                    .ConfigureAwait(false);
            }
            finally
            {
